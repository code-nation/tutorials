...
$(document).ready(function(){
  ...
  (function(){
    ...
    this.updateProgressStages = function(stages, direction) {
      ...
      var next = current.next();
      var previous = current.prev();
      var donation_btn_amount = $('.progress-stage input[type="radio"]:checked').length == 0 ? 0 : parseInt($('.progress-stage input[type="radio"]:checked').val());
      StagedDonations.showDonationAmount(donation_btn_amount);
      if (direction && current.next().length) { // direction = 0 if going backwards, 1 if going forwards
    ...
    this.showDonationAmount = function(donation_btn_amount){
      var donate_value = $('#donation_amount_other').val() == "" ? donation_btn_amount :  parseFloat($('#donation_amount_other').val());
      $('#chosen_donation_amount').text("Donation amount: $" + donate_value);
    }
  ...
  if ($('.progress-indicator-stages').length) {
    ...
    $('.progress-stages .radio').each(function(){$(this).click(function(event){
      StagedDonations.updateProgressIndicator(progressIndicator, 1);
      StagedDonations.updateProgressStages(progressStages, 1);
      var donation_btn_amount = parseInt($(this).prev('input[type="radio"]').val());
      StagedDonations.showDonationAmount(donation_btn_amount);
    });});
